  ...
  <div class="slider-wrapper w-full overflow-hidden">
    <div class="w-full h-120 flex-row relative" @mouseenter="stopInterval" @mouseleave="startInterval">
      ...
      <div class="bullets absolute top-0 h-full w-full flex justify-center items-end py-2">
        <div class="relative">
          <ul class="list-disc grid grid-flow-col gap-6 text-2xl absolute bottom-0 z-10">
            <li class="cursor-pointer" :class="[ currentSlide == count - 1 ? 'text-gray-600 shadow-lg' : 'text-white']" @click="setCurrentSlide(count - 1)" v-for="count in images.length" :key="count"></li>
          </ul>
        </div>
      </div>
      <!--Bullets-->
